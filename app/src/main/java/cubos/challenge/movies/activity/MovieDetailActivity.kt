...
class MovieDetailActivity : AppCompatActivity(), MainView {
    ...
    override fun setMovieDetailData(movie: Movie) {
        val downloadPosterTask = DownloadPosterTask(movieDetailPosterImageView, movieDetailImageProgressBar, movie)
        val finalPosterURL = BASE_IMAGE_URL.plus(IMAGE_QUALITY_HIGH).plus(movie.posterPath)
        title = movie.title
        movieDetailDescriptionTextView.text = movie.overview
    ...
    override fun hideInfinityScrollProgress() { }

    override fun showInfinityScrollProgress() { }
